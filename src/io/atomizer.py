...
class Atomizer:
    ...
    def packet_from_payload(self, payload: bytes, boundary: str = "generated") -> AtomPacket:
        ...
        When ``boundary`` is the default ``generated`` tag, infer a structural
        boundary from the payload tail so rolling context features stay closer
        to packets produced by ``encode`` during training.
        """
        if not payload:
        ...
        if boundary == "generated":
            boundary = self._infer_boundary(payload)
        return self._emit(bytes(payload), self.position, boundary)

    @classmethod
    def _infer_boundary(cls, payload: bytes) -> str:
        if not payload:
            return "eos"
        last = payload[-1]
        if last in (10, 13):
        ...
        if last in (9, 32):
            return "whitespace"
        if len(payload) >= 2:
            # Mimic encode()'s class-transition cut for alnum switches.
            prev, cur = payload[-2], payload[-1]
            if not cls._is_continuation_byte(cur):
                pc, cc = cls._class(prev), cls._class(cur)
                if pc != cc and {pc, cc} <= {"alpha", "digit"}:
                    return "class_transition"
        return "max_span"
