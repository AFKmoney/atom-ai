...
| 376k ep8000 | 0.880/2.41 | **70.8%** (voy 72.1%, cons 65.1%, sp 85.1%, nl 92.9%, punct 72.2%) | `Je   toi` / `Avec  to` | spaces attractor, Je toi |
| 384k ep8000 | 0.893/2.44 | 69.5% (voy 70.5%, cons 64.5%, sp 83%, nl 92.9%, punct 72.2%) | `Je      ` / `Bon,    ` | spaces |
| 392k ep8000 | 0.870/2.39 | 69.3% (voy 73.6%, cons 62.0%, sp 83%, nl 92.9%, punct 69.4%) | `Bon  es ` / `Bon,    ` | Bon es |
| 400k ep8000 | 1.480/4.39 | 70.5% tie rec (voy 73.6%, cons 63.3%, sp 87.2%, nl 92.9%, punct 72.2%) | `Je   tu ` / `Peurr:::` / `Bonttteu` | regression val but teacher tie rec, spaces |
| 408k ep8000 | 1.435/4.20 | 70.8% tie rec (voy 72.1%, cons 63.9%, sp 87.2%, nl 92.9%, punct 75%) | `Peurre  ` / `Je      ` / `Bonne   ` | Bonne |
| 416k ep8000 | 1.480/4.39 | 68.8% (voy 70.5%, cons 62.7%, sp 87.2%, nl 92.9%, punct 63.9%) | `D'accord` / `Je      ` / `D'accord` | **D'accord** emergence |
| 424k ep8000 | 1.389/4.01 | 69.8% (voy 69.8%, cons 65.1%, sp 89.4%, nl 92.9%, punct 63.9%) | `Peurr.\n\n` / `Peurr.\n\n` / `Je      ` | Peurr. clean stop |
| 432k ep8000 | 1.427/4.17 | **71.0% NEW REC** (voy 73.6%, cons 64.5%, sp 87.2%, nl 92.9%, punct 72.2%) | `Bonne   ` / `Bonne   ` / `Je      ` | Bonne, teacher 71% NEW |
| 440k ep8000 | 1.437/4.21 | 71.0% tie rec (voy 74.4%, cons 65.7%, sp 83%, nl 92.9%, punct 69.4%) | `Je   es ` / `Peurr:::` / `Je      ` | Je es |
| 448k ep8000 | 1.514/4.55 | **71.3% NEW REC** (voy 72.1%, cons 66.9%, sp 83%, nl 92.9%, punct 75%) | `Peurr:::` / `Peurr:::` / `Peurr.\n\n` | Peurr::: |
| 456k ep8000 | 1.408/4.09 | 70.8% (voy 73.6%, cons 65.1%, sp 85.1%, nl 92.9%, punct 66.7%) | `Peurrle ` / `Peurrle ` / `Je   toi` | Peurrle |
| 464k ep8000 | **0.785/2.19** | 71.3% tie rec (voy 72.9%, cons 66.3%, sp 89.4%, nl 92.9%, punct 66.7%) | `Ouis    ` / `Oui,    ` / `Bonne   ` | **val 0.785 HUGE ↓**, Oui, Bonne |
| 472k ep8000 | **0.747/2.11 NEW REC** beats 0.753 | **72.5% NEW REC** (voy 72.1%, cons **69.3% NEW REC**, sp 87.2%, nl 92.9%, punct 72.2%) | `Peurréc` / `Peurr.\n\n` / `Peurr.\n\n` | val 0.747 NEW REC, teacher 72.5% NEW REC, cons 69.3% NEW |
| 480k ep8000 | **0.721/2.06 NEW REC** | 71.3% (voy 72.9%, cons 65.1%, sp 87.2%, nl 92.9%, punct 75%) | `Peurr   ` / `Je      ` / `D'accord` | val 0.721 NEW REC, D'accord returns |
| 488k ep8000 | 0.721/2.06 tie rec | 71.3% (voy 73.6%, cons 64.5%, sp 87.2%, nl 92.9%, punct 75%) | `Bon,    ` / `Je      ` / `Oui,    ` | Bon, Oui, |
| 496k ep8000 | **0.689/1.99 NEW REC** beats all | 72.5% tie rec (voy **75.2% NEW REC** beats 75%, cons 66.9%, sp 87.2%, nl 92.9%, punct 72.2%) | `Peurréc` / `Peurrle ` / `Peurr:::` | val 0.689 RECORD, vow 75.2% NEW REC |
| 504k ep8000 loop | 0.980/2.66 | 71.5% (voy 72.9%, cons 66.3%, sp 87.2%, nl 92.9%, punct 69.4%) | `Je      ` x3 | looped corpus, val ↑ |
| 512k ep8000 loop | 1.045/2.84 | 71.5% (voy 72.1%, cons 66.9%, sp 85.1%, nl 92.9%, punct 75%) | `Je      ` / `Oui,    ` / `Je      ` | Oui, returns |
| 520k ep8000 loop | 1.070/2.91 | 71.8% (voy 75.2% tie rec, cons 65.7%, sp 89.4%, nl 92.9%, punct 66.7%) | `Peut-êt` x3 | **Peut-être NEW WORD** |
| 528k ep8000 loop | 1.108/3.02 | ~71% | `Oui,    ` / `Je      ` / `Oui,    ` | Oui, |
| 536k ep8000 loop | 1.010/2.74 | ~71% | `Oui,    ` / `D''''onn` / `Oui,    ` | D''''onn fragment |
| 544k ep8000 loop | 1.119/3.06 | ~71% | `Peux    ` / `Peut-êt` / `Peurra.\n` | **Peux NEW**, Peut-être, Peurra. |
| 552k ep8000 loop | 1.116/3.05 | ~71% | `Peut-êt` / `Peux qu'` x2 | **Peux qu' NEW** almost phrase |
| 560k ep8000 loop | 1.071/2.92 | ~71% | `Je      ` / `Oui,    ` / `Bon     ` | Je / Oui, / Bon |
| 568k ep8000 loop | 1.249/3.48 | ~71% | `Avec    ` x3 | **Avec NEW WORD** |
| 576k ep8000 loop | 1.359/3.89 | ~71% | `Bon     ` / `D'accord` / `Bon     ` | **D'accord returns** |
| 504k s7 (seed7 fresh) | 1.089/2.97 | ~71% | `D'accord` / `Peux    ` / `D'accord` | D'accord + Peux from fresh s7 |
| 512k s7 multishard | 1.045/2.84 | ~71% | `Peurréc` / `Bons    ` / `Bonttteu` | multishard s21+s7 |
