...
class AtomNativeModelTests(unittest.TestCase):
    ...
    def test_default_energy_decay_floor(self) -> None:
        """Always-on band must catch near-zero decay without explicit bounds."""
        from src.atom_native import DEFAULT_ENERGY_DECAY_BOUNDS
        ...
        self.assertEqual(model.energy_decay_bounds, DEFAULT_ENERGY_DECAY_BOUNDS)
        with torch.no_grad():
            model.core.dynamics.dynamics.energy_decay.fill_(0.001)
        state = model.stabilize_dynamics_parameters()
        self.assertAlmostEqual(state["energy_decay"], DEFAULT_ENERGY_DECAY_BOUNDS[0], places=5)
        self.assertTrue(state["energy_decay_repaired"])
