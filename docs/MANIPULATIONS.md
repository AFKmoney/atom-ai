# Manipulations — chronological repair log / Journal chronologique
...
**EN** narrative first, **FR** summary after each major phase.

---

## 0. Context / Contexte

**EN.** Starting point: a toroidal research repo that had drifted toward GPT-2
...
**FR.** Point de départ : dérive GPT-2 / HF, docs bruitées. Objectif : spine
atom-native seule, docs honnêtes, corpus dialogue, correctifs gen, train long.
...
**FR.** Quarantaine des chemins GPT-2 et des docs-slop ; CLI atom-native ;
pas d'attention dans le cœur toroïdal.
...
**FR.** Outils train/chat atom-native ; docs et tests alignés sur l'Atomizer.
...
**FR.** Corpus dialogue FR ; export atom-ai = petits échantillons seulement.
...
**FR.** Premier train CPU ; chat encore bruit d'octets — dit sans marketing.

---

## 5. Generation bugs diagnosed (2026-09-16 → 17 PT)

**EN.** Root causes (`GEN_DEBUG.md`):
...
**FR.** Biais de surface, reset 64, boundary `generated` → correctifs LayerNorm,
épisode 512, champ persistant, tests smoke.
...
**FR.** +250k pas ; diversité de prompts OK ; français embryonnaire ; decay malade.
...
**FR.** Persist jusqu'à ~1,05M pas ; meilleur ckpt chat ~8 Mo exporté ici.
...
**FR.** Clamp `energy_decay ∈ [0.3, 0.95]` toujours actif (step + load).

---

## 9. Clean export → GitHub `atom-ai` (2026-09-17 PT)

**EN.** This tree (`/workspace/exports/atom-ai/`):
...
**FR.** Export propre `atom-ai` ; pas de push depuis cet agent ; docs FR+EN.
